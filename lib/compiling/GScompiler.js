...
*/
	...
	var classpatt = new RegExp('class\\s*(\\w+)\\s*:') // find a Python class
	var defpatt = new RegExp('def\\s*(\\w+)\\s*\\(')   // find a Python def
	var asyncpatt1 = new RegExp('\\Wfunction\\s*(\\w+)\\s*\\(')      // find "function f(...."
	//var asyncpatt2 = new RegExp('\\W(\\w+)\\s*=\\s*function\\s*\\(') // find "f = function (..." (which is converted to "function f(..."
	var awaitpatt = new RegExp('\\W(\\w+)\\s*\\(')  // find a function call
	var jsasyncpatt1 = new RegExp('\\Wfunction\\s*([\\w\.]+)\\s*\\(')      // find "function f(...."
	var jsasyncpatt2 = new RegExp('\\W([\\w\.]+)\\s*=\\s*function\\s*\\(') // find "f = function (..."
	...
	function preprocess(program, lang) { // manipulate the program source
		...
		for (lineno=0; lineno<lines.length; lineno++) { // process individual lines of code
    		...
    		if (lang != 'javascript') {
				...
				var newpatt = new RegExp('\\Wnew\\s*([\\w\\.]+)\\s*\\(') // new f()
    ...
    function compile(program, options) {
		...
		// } else {
		// 	for (var c of classes) if (fcts.indexOf(c) >= 0) 
		// 		throw new Error('In GlowScript JavaScript it is not currently possible to use "new" with your own functions.')
		// }
		...
		while (true) {
			...
			if (m === null) break
			name = m[1]
			if (fcts.indexOf(name) < 0 || classes.indexOf(m[1]) >= 0) { // fcts is a list of user functions plus GlowScript async functions such as rate
				// We also check that this isn't a class constructor
